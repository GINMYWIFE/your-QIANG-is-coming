        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                var spec = chatClient.prompt()
                    .system(s -> s.text(systemPromptResource).params(systemParams));
                
                if (userPromptResource != null) {
                    spec = spec.user(u -> u.text(userPromptResource).params(userParams));
                } else if (userParams != null && userParams.containsKey("userPrompt")) {
                    spec = spec.user(userParams.get("userPrompt").toString());
                }

                return spec.call().entity(outputConverter);
            } catch (Exception e) {
                lastError = e;
