...
export default function InterviewConfigPanel({
  ...
  const questionCounts = [1, 3, 5, 8, 10];
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {knowledgeBases.map((kb) => (
                <button
                  key={kb.id}
                  onClick={() => onKnowledgeBaseChange(kb.id)}
                  className={`px-4 py-3 rounded-xl border-2 transition-all text-left flex flex-col gap-1 ${
                ...
                >
                  <span className="font-semibold truncate">{kb.name}</span>
                  <span className="text-xs opacity-70 truncate">{kb.category || '通用岗位'}</span>
                </button>
              ))}
              {knowledgeBases.length === 0 && (
                <div className="col-span-full p-4 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-slate-400">
                  暂无可用岗位知识库，请先前往知识库管理上传
                </div>
              )}
            </div>
          </div>
